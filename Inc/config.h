...
#define DEFAULT_RATE                150   // 30.0f [-] lower value == slower rate [0, 32767] = [0.0, 2047.9375]. Do NOT make rate negative (>32767)
#define DEFAULT_FILTER              1500  // Default for FILTER 0.1f [-] lower value == softer filter [0, 65535] = [0.0 - 1.0].
#define DEFAULT_SPEED_COEFFICIENT   16384 // Default for SPEED_COEFFICIENT 1.0f [-] higher value == stronger. [0, 65535] = [-2.0 - 2.0]. In this case 16384 = 1.0 * 2^14
...
// #define DUAL_INPUTS                     // ADC*(Primary) + PWM(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define FLASH_WRITE_KEY       0xF00C  // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
    #define FEEDBACK_ROBO		// aditional runtime data like current and odometer
    // #define FEEDBACK_SERIAL_USART2      // left sensor board cable, disable if ADC or PPM is used!
    #define DEBUG_SERIAL_USART2           // left sensor cable debug
    // #define FEEDBACK_SERIAL_USART3      // right sensor board cable, disable if I2C (nunchuk or lcd) is used!
...
// ########################### UART SETIINGS ############################
  ...
  #ifndef USART3_BAUD
    #define USART3_BAUD           115200                  // ROBO 115200 UART3 baud rate (short wired cable)
  #endif
